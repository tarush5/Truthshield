...
def get_current_user(
    ...
    #
    # The asymmetric branch here used to decode with verify_signature disabled,
    # "to avoid PEM loading errors". That accepted any RS256/ES256/PS256 token
    # whose payload an attacker chose: forge {"sub": ..., "email": "someone@
    # else"}, sign it with your own key, and every endpoint behind
    # get_current_user treated you as that user. Verifying an asymmetric token
    # needs the provider's public keys, which this server does not fetch, so
    # such tokens are now refused.
    if settings.SUPABASE_JWT_SECRET:
        try:
            ...
            if not alg.startswith("HS"):
                logger.warning(
                    "Rejecting token signed with %s: asymmetric signatures cannot be "
                    "verified against the symmetric SUPABASE_JWT_SECRET", alg
                )
                payload = None
            else:
                payload = jwt.decode(
