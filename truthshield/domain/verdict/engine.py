"""
Verdict engine — decides a claim against retrieved evidence.

Ported from the previous implementation rather than rewritten. These two
modules carry a long tail of fixes that were each found by running the system
against real evidence and measuring the result — anchored domain matching,
stem-based relevance, sentence-scoped negation, typographic apostrophes, the
two-character token floor that made "5G" visible, the question-headline guard,
and the minimum-evidence rule. Retyping them would have quietly reintroduced
the bugs they exist to prevent.

What changed in the port: imports now resolve inside `truthshield`, and the
public surface is adapted to the domain types in `truthshield.domain.types`
via `adapters.py`.
"""
...
from truthshield.domain.verdict.llm_config import (
...
from truthshield.domain.verdict.legacy_types import Claim, ClaimVerdict, Evidence, Verdict
...
Analyze this claim and provide your verdict as JSON."""
    ...
    def _tfidf_evaluate(self, claim: Claim, evidence: List[Evidence]) -> ClaimVerdict:
        ...
        if relevant == 0:
            ...
            parts.append(_plural(refute_items, "source contradicts", "sources contradict") + " this claim.")
        elif support > contra * 1.2:
            verdict = Verdict.TRUE
            ratio = support / max(contra, 0.01)
            confidence = min(0.92, 0.45 + min(0.47, support * 0.35))
            parts.append(_plural(support_items, "source supports", "sources support") + " this claim.")
        ...
        # Source titles are deliberately NOT listed here.
        #
        # They were, and the result read badly: the titles are truncated to 60
        # characters, so the sentence came out as "Refuting sources: Report: No
        # evidence bleach consumed to cure COVID-19 during ; Perspective |
        # Garlic and bleach won't cure coronavirus. How ; ..." — mid-word cuts,
        # stray separators, and the same source repeated. Every source is
        # already rendered underneath the claim with its real title, its
        # stance and a link, so restating three mangled ones added nothing.
        if relevant:
            parts.append(
                f"Based on {relevant} relevant "
                f"{'source' if relevant == 1 else 'sources'} "
